...
Методология discovery-интервью (шаблоны, скилл, контракт brief); исполняемый
runtime живёт в соседнем репо `discovery`.

## Repo scope & boundaries

- **Этот репо:** `discovery-toolkit` — git-корень `all_ai_orchestrators/discovery-toolkit/`, remote `git@github.com:andrei-shtanakov/discovery-toolkit.git`.
